#!/usr/bin/env python3
"""
TÜM GUI'Yİ TAMAMEN İKİ DİLLİ HALE GETİREN BETİK
- app/main.py'yi yenisiyle değiştirir
- translations.json'u günceller
"""
import os, json
...
# ──────────────────────────────────────────────
# YENİ TRANSLATIONS.JSON İÇERİĞİ
# ──────────────────────────────────────────────
TRANSLATIONS = {
    ...
    "lang_en": {"tr": "🇬🇧 English", "en": "🇬🇧 English"},
    "tempering_time": {"tr": "Süre (s):", "en": "Time (s):"},
    "carbon_potential": {"tr": "C Potansiyeli (%):", "en": "C Potential (%):"},
    ...
    "lever_info": {"tr": "<h2>⚖️ Kaldıraç Kuralı (Lever Rule)</h2>İki fazlı bölgede faz oranlarını hesaplayın.", "en": "<h2>⚖️ Lever Rule</h2>Calculate phase fractions in two-phase regions."},
    "lever_initial_result": {"tr": "<i>Değerleri girin ve Hesapla'ya basın...</i>", "en": "<i>Enter values and press Calculate...</i>"},
    ...
    "ree_title": {"tr": "<h2>🧪 Nadir Toprak Elementleri (REE)</h2>", "en": "<h2>🧪 Rare Earth Elements (REE)</h2>"},
    "ree_applications_tab": {"tr": "Uygulamalar", "en": "Applications"},
    "ree_usage_tab": {"tr": "Kullanım Dağılımı", "en": "Usage Distribution"},
    "ree_minerals_tab": {"tr": "Mineraller & İşleme", "en": "Minerals & Processing"},
    "ree_usage_title": {"tr": "<h3>ABD Nadir Toprak Kullanımı</h3>", "en": "<h3>US Rare Earth Usage</h3>"},
    "ree_minerals_title": {"tr": "<h3>REE Mineralleri</h3>", "en": "<h3>REE Minerals</h3>"},
    "ree_extraction_title": {"tr": "<br><h3>Ekstraksiyon Aşamaları</h3>", "en": "<br><h3>Extraction Steps</h3>"},
    "ree_separation_note": {"tr": "<br><b>⚠️ Zorluk:</b>", "en": "<br><b>⚠️ Challenge:</b>"},
    "failure_title": {"tr": "Malzeme Hata Modlarına Giriş", "en": "Introduction to Material Failure Modes"},
    "failure_intro": {"tr": "Her mühendislik malzemesinin sınırları vardır.", "en": "Every engineering material has its limits."},
    "failure_fractography_title": {"tr": "Kırık Yüzeyi Analiz Araçları (Fraktografi)", "en": "Fracture Surface Analysis Tools (Fractography)"},
    "failure_prevention_title": {"tr": "Önleme Stratejileri", "en": "Prevention Strategies"},
    "failure_modes_fracture_title": {"tr": "💥 Kırılma: Uyarılı veya Uyarısız", "en": "💥 Fracture: With or Without Warning"},
    "failure_modes_fatigue_title": {"tr": "🔄 Yorulma: Tekrarlı Gerilme", "en": "🔄 Fatigue: Repeated Stress"},
    "failure_modes_creep_title": {"tr": "⏳ Sürünme: Zamana Bağlı Şekil Değiştirme", "en": "⏳ Creep: Time-Dependent Deformation"},
    "failure_modes_corrosion_title": {"tr": "🧪 Korozyonla İlişkili Hatalar", "en": "🧪 Corrosion-Related Failures"},
    "failure_modes_impact_title": {"tr": "💢 Şok Yükleme Nedeniyle Darbe Hatası", "en": "💢 Impact Failure from Shock Loading"},
    "failure_modes_wear_title": {"tr": "🔧 Aşınma: Yüzey Hasarı", "en": "🔧 Wear: Surface Damage"},
}

# ──────────────────────────────────────────────
# YENİ app/main.py İÇERİĞİ (uluslararasılaştırılmış)
# ──────────────────────────────────────────────
NEW_MAIN = r'''#!/usr/bin/env python3
...
class MainWindow(QMainWindow):
    ...
    def _ui(self):
        ...
        self.diff_temp = QLineEdit("930"); diff_form.addRow(self.lang_manager.tr("diffusion_temperature"), self.diff_temp)
        self.diff_time = QLineEdit("2"); diff_form.addRow(self.lang_manager.tr("diffusion_time"), self.diff_time)
        self.diff_cs = QLineEdit("0.8"); diff_form.addRow(self.lang_manager.tr("diffusion_cs"), self.diff_cs)
        self.diff_c0 = QLineEdit("0.2"); diff_form.addRow(self.lang_manager.tr("diffusion_c0"), self.diff_c0)
        self.diff_depth = QLineEdit("5"); diff_form.addRow(self.lang_manager.tr("diffusion_depth"), self.diff_depth)
        diff_btn = QPushButton(self.lang_manager.tr("diffusion_simulate")); diff_btn.clicked.connect(self._simulate_diffusion)
        ...
        self.tabs.addTab(diff_tab, self.lang_manager.tr("diffusion_tab"))
...
# UYGULA
# ──────────────────────────────────────────────
with open(os.path.join(BASE, "database", "translations.json"), 'w', encoding='utf-8') as f:
    json.dump(TRANSLATIONS, f, indent=2, ensure_ascii=False)
print("✅ translations.json güncellendi")

with open(os.path.join(BASE, "app", "main.py"), 'w', encoding='utf-8') as f:
    f.write(NEW_MAIN)
print("✅ app/main.py tamamen uluslararasılaştırıldı")

print("\n🎉 İşlem tamam! Şimdi python main.py ile çalıştırın.")