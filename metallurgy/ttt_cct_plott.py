...
# ============================================================
# ESKİ FONKSİYONLAR (GUI TTT/CCT sekmesi için gerekli)
# ============================================================
def load_ttt_data(steel_name):
    """Veritabanından TTT verisini yükler."""
...
def draw_ttt(ax, ttt_data):
    """TTT diyagramını matplotlib eksenine çizer."""
    if not ttt_data:
...
# ============================================================
# YENİ JMatPro FONKSİYONLARI
# ============================================================
def calculate_ttt(composition, grain_size=10):
    """
    Alaşım bileşimine göre TTT diyagramı parametrelerini hesaplar.
    composition: {'C':0.4, 'Mn':0.8, ...} (ağırlıkça oran)
    """
    ...
    return {
        'pearlite': {'T': T_pearlite.tolist(), 't': tau_pearlite.tolist()},
        'bainite': {'T': T_bainite.tolist(), 't': tau_bainite.tolist()},
        'Ms': Ms
    }

def cct_from_ttt(ttt_data, cooling_rates):
    """TTT verisinden CCT diyagramı tahmini yapar."""
...
def hardness_from_cct(cooling_rate, composition):
    """Soğuma hızı ve bileşime göre sertlik tahmini yapar (HV)."""
    C = composition.get('C', 0) * 100
    hardness = 200 + 300 * C + 5 * np.log10(cooling_rate + 1)
    return min(hardness, 700)  # Maksimum 700 HV